...
def motor_move_speed(N, spd, packet):
    ...
    packet.append(0x01)  ##模式 0x01為速度控制模式
    packet.append(N)  ##幾號機
    packet.append(spd2)  # RPM上限定義
    packet.append(spd1)  # RPM上限定義
    packet.append(0x00)  #無意義
    packet.append(0x00)
    packet.append(0x00)
    packet.append(0x00)
...
for spd in range(20):
    ...
    motor_move_speed(1, 20+spd,packet) #(N號馬達,轉速(RPM)，空的陣列(用於儲存指令碼)) 如要反轉 就把轉速參數 加上一個-號
    time.sleep(0.2)
    packet = bytearray()
    motor_move_speed(2, -20-spd, packet)  # (N號馬達,轉速(RPM)，空的陣列(用於儲存指令碼)) 如要反轉 就把轉速參數 加上一個-號
    #print("一號輪=" , spd ,"二號輪=" , -spd)
    time.sleep(1)
    print(spd)
for spd in range(20):
    packet = bytearray()#創一個空的陣列 類型是bytearray
